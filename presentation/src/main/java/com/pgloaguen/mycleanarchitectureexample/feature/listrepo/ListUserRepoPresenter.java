...
import com.pgloaguen.mycleanarchitectureexample.utils.rx.MapWithIndex;
...
public class ListUserRepoPresenter {
    ...
    private void loadRepo(String username, boolean invalidate) {
        ...
        getUserRepoUseCase.execute(GetUserRepo.Param.create(username, invalidate))
                ...
                .subscribe(vm -> {
                    if (vm.value().error == null) {
                        stateMachine.nextState(vm.index() == 0 ? NEW_DATA : DATA_UPDATE, __ -> vm.value());
                    } else {
                        stateMachine.nextState(ERROR, it -> new VM(it == null ? Collections.emptyList() : it.repoEntities, vm.value().error));
                    }
                });
    }
}
